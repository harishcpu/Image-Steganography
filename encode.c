...
int main(int argc, char *argv[])
{
    ...
    system("clear");
    printf("\t\t\t\t" BOLDRED "STEGANOGRAPHY" RESET "\n\t\t\t\t-------------\n");
    ...
    if (Optype == e_encode)
        ...
        printf(BOLDRED "INFO" RESET ":" BOLDRED " Unspecified Flag\n" RESET);
        return e_failure;
    }
...
Status encode(char *argv[])
{
    ...
    printf(BOLDRED "INFO" BOLDGREEN ": ## ENCODING PROCEDURE STARTED ##" RESET "\n");


    /* check for the secret file size */
    printf(BOLDRED "INFO" RESET ": Checking for " BOLDMAGENTA "%s" RESET "\'s size\n",\
            encInfo.secret_fname);
    if (get_file_size(encInfo.fptr_secret, &(encInfo.size_secret_file)))
        printf(BOLDRED "INFO" RESET ": Done. Not Empty\n");
    else {
        printf(BOLDRED"INFO" RESET ": Done. File is Empty. Encoding is not possible\n");
        printf(BOLDRED"INFO" RESET ": " BOLDYELLOW "ABORTING ENCODING PROCEDURE\n" RESET);
        printf(BOLDRED "INFO" RESET": " BOLDRED "###      ENCODING ABORTED      ###" RESET);
...
//printf("%ld\n", encInfo.size_secret_file);
    ...
    printf(BOLDRED "INFO" RESET ": Checking for " BOLDMAGENTA "%s" RESET " capacity to handle secret.txt\n", encInfo.src_image_fname);
    if (e_success == check_capacity(&encInfo)) {
        printf(BOLDRED "INFO" RESET ": Done. Found " BOLDGREEN "OK" RESET "\n");
        if (argv[4] == NULL)
            printf(BOLDRED "INFO" RESET ": Output File not mentioned. Creating " \
                    BOLDMAGENTA "steged_img.bmp" RESET " as default\n");
    } else {
        printf(BOLDRED "INFO" RESET ": Done. Found " BOLDRED " NOT OK" RESET"\n");
        printf(BOLDRED"INFO" RESET ": " BOLDYELLOW "ABORTING ENCODING PROCEDURE\n" RESET);
        sleep(1);
        printf(BOLDRED "INFO" RESET": " BOLDRED "ENCODING ABORTED\n" RESET);
        return e_failure;
    }

    /* Copy image header */
    printf(BOLDRED "INFO" RESET ": Copying Image Header\n");
    if (e_success == copy_bmp_header(encInfo.fptr_src_image, encInfo.fptr_stego_image))
        printf(BOLDRED "INFO" RESET ": " BOLDGREEN "Done" RESET "\n");
    else
        printf(BOLDRED "INFO" RESET ": " BOLDRED "Failed" RESET "\n");

    /* Encode Magic String Signature */
    printf(BOLDRED "INFO" RESET ": Encoding Magic String Signature\n");
    ...
    if (*(*(argv + 1) + 2) == 'p') {
        printf(BOLDRED "INFO" RESET ": Encoding Length of PassCode\n");
        if (e_success == encode_passcode_length(&encInfo))
            printf(BOLDRED "INFO" RESET ": " BOLDGREEN "Done" RESET "\n");

        printf(BOLDRED "INFO" RESET ": Encoding PassCode\n");
        if (e_success == encode_passcode(&encInfo))
            printf(BOLDRED "INFO" RESET ": " BOLDGREEN "Done" RESET "\n");
    }

    /* Encode secret file extension */
    printf(BOLDRED "INFO" RESET ": Encoding " BOLDMAGENTA "%s" RESET " File Extension\n", encInfo.secret_fname);
    ...
    printf(BOLDRED "INFO" RESET ": Encoding " BOLDMAGENTA "%s" RESET " File Size\n", encInfo.secret_fname);
    ...
    printf(BOLDRED "INFO" RESET ": Encoding " BOLDMAGENTA "%s" RESET " File Data\n", encInfo.secret_fname);
    ...
    printf(BOLDRED "INFO" RESET ": Copying Left Over Data\n");
    if (e_success == copy_remaining_img_data(encInfo.fptr_src_image, encInfo.fptr_stego_image))
        printf(BOLDRED "INFO" RESET ": " BOLDGREEN "Done" RESET "\n");
    else
        printf(BOLDRED "INFO" RESET ": " BOLDRED "Failed" RESET "\n");
    
    printf(BOLDRED "INFO" RESET ": " BOLDGREEN " ## ENCODING DONE SUCCESSFULLY ##\n"); 
